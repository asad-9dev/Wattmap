...
export type Band = { tone: BandTone; label: string; min: number; max: number };

/**
 * Three bands per metric. Percentile and score use fixed thirds; EUI and GHG intensity use the
 * tertiles of the schools currently shown, since they have no natural fixed scale. Colours come
 * from the theme's signal tokens (see MapLegend and SchoolMap), never from the band itself.
 */
export function bandsFor(metric: MapMetric, values: number[]): Band[] {
  const digits = metric === "eui" ? 2 : metric === "ghgIntensity" ? 1 : 0;
  const cuts: [number, number] =
    metric === "percentile" || metric === "score" ? [33.3, 66.7] : [quantile(values, 1 / 3) ?? 0, quantile(values, 2 / 3) ?? 0];
  const low = Math.min(0, ...values);
  const high = Math.max(100, ...values);
  const f = (v: number) => formatNumber(v, digits);
  return [
    { tone: "low", label: `Lower (below ${f(cuts[0])})`, min: low, max: cuts[0] },
    { tone: "mid", label: `Middle (${f(cuts[0])}–${f(cuts[1])})`, min: cuts[0], max: cuts[1] },
    { tone: "high", label: `Higher (above ${f(cuts[1])})`, min: cuts[1], max: high },
  ];
}
